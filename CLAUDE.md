...
**Intelligence engines are pure.** `@dsa/intelligence` contains five engines — topic priority, revision (SM-2), weakness, difficulty, roadmap — plus an `IntelligenceOrchestrator`. They take snapshot data in and return results with **no I/O, no DB, no network**. The backend feeds them data and persists outcomes. Keep them that way: this is what makes them unit-testable in isolation, and most of the test suite lives here.
