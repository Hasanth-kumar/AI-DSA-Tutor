...
function setup(db: TestDb): void {
  ...
  // A local embedding for A ÿÿÿ must never reach the sync target (?6).
  upsertEmbedding(db, {
    cardId: "A",
    model: "fake",
...
describe.skipIf(!DatabaseSync)("Stage-6 card sync (?8, ?10)", () => {
  ...
  it("keeps code-heavy cards local-authoritative on pull ÿÿÿ pointer never clobbers local content (?8)", async () => {
    const db = freshDb();
    db.prepare(`INSERT INTO topics(id,name,updated_at) VALUES(?,?,1)`).run(TOPIC, "Two Pointers");
    ...
    // code-heavy flag ÿÿÿ content is local-authoritative and must be preserved.
      ...
      [
        {
          ...
          back: "? code / long content in page body ÿÿÿ card kept local-authoritative (?8)",
          conceptTags: ["overflow"],
          codeHeavy: true,
        },
      ],
    ...
    // Local content is intact ÿÿÿ the pointer did NOT overwrite the real code body.
