...
- [x] **Code-heavy content lives in the card body / page blocks**, not Notion property fields (avoids length/formatting limits) — or such cards are kept local-authoritative. *(Both halves of the OR are now implemented and observed. `isCodeHeavy` flags cloze cards, fenced-code, or over-2000-char content. On push: `cardToNotionPageBlocks` emits the full Front+Back as Notion `code` blocks in the page **body**, while `cardToNotionProperties` writes only `CODE_IN_BODY_NOTICE` into the length-limited `Back` property; `NotionSyncTarget` attaches `children` only for these cards. All other content is chunked into ≤2000-char text items so no push is rejected. On pull: `notionPageToContent`/`recordToContent` set `codeHeavy`, and `applyPulledContent` keeps such cards **local-authoritative** — the pointer never overwrites local content (only the page mapping is rebound). Observed: `card-properties.test.ts` (14 tests, 6 new) + `card-sync.test.ts` code-heavy case (real node:sqlite: local code body preserved, SR state untouched, `updated_at` not bumped) all pass.)*
